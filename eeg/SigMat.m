classdef SigMat < handle
   ...
   % class, which allows to read from file only parts of saved arrays,
   % thereby allowing to manage very long recording sessions without having
   % to split data into several .MAT files.
   %
   % While SigMat offers some convenience methods, nothing prevents users
   % from directly accessing data stored in SigMat-generate files, as they
